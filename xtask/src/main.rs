...
fn run_kats() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let wrapper = target.join(profile).join("hqckem_wrapper");
    let katwalk = tools.join("bin/katwalk");
    let fips_vectors = vectors.join("vectors/custom/FIPS-207-pre-ipd");
    run_kat(
        &katwalk,
        &wrapper,
